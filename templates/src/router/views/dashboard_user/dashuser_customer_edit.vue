...
export default {
  ...
  methods: {
    ...
    updateCustomer: async function() {
      // console.log(this.id);
      const { data } = await CustomerRepository.update(this.customer);
      this.customer = data;

      this.$router.push("/customer");
    },
    fetchCustomer: async function(id) {
      const { data } = await CustomerRepository.getCustomer(id);
          ...
          <div class="card">
            <div class="card-body">
              <form action="#" v-on:submit.prevent>
                ...
                <div class="buttonbox text-left">
                  <button type="button" class="btn btn-secondary mr-3">Discard</button>
                  <button type="submit" v-on:click="updateCustomer()" class="btn btn-primary">Save</button>
                </div>
              </form>
            </div>
          </div>
